...
if [[ $CLUSTER_TYPE == "aws" ]]; then
  aws secretsmanager create-secret --name "gi-$SECRET_TYPE-secret-$RANDOM_STR" --region $DEPLOY_REGION --secret-string "file://$SECRETFILE"
  log "Secret for $SECRET_TYPE created in AWS Secrets Manager"
elif [[ $CLUSTER_TYPE == "azure" ]]; then
  # Check if key vault already exists
  vaultname=gi-vault-$RANDOM_STR
  ...
  output=$(az keyvault secret set --name gi-$SECRET_TYPE-secret --vault-name $vaultname --file $SECRETFILE 2>&1)
  if [[ $? -ne 0 ]]; then
    ...
    sleep 5
    az keyvault secret set --name gi-$SECRET_TYPE-secret --vault-name $vaultname --file $SECRETFILE 2>&1
  fi
  # az keyvault secret show --name gi-$SECRET_TYPE-secret --vault-name $vaultname
  log "Secret created in Azure Key Vault"
elif [[ $CLUSTER_TYPE == "gcp" ]]; then
  gcloud secrets create "gi-$SECRET_TYPE-secret-$RANDOM_STR" --data-file=$SECRETFILE
